...
const files = [
  ...
  '80c552/logging.js', '80c552/uart.js', '80c552/80c552.js',
];
...
## Full UART / service port support

`uart.js` gives you the on-chip serial port (S0CON/S0BUF) at the byte
level - TI/RI, TB8/RB8, REN all behave like real hardware; baud-rate
generation itself isn't modeled (same instant-completion approach as
everything else here). Installed automatically by `create_80c552()` as
`cpu.uart`.

```js
cpu.uart.onTx((byte, ninthBit) => console.log('TX', byte.toString(16)));
cpu.uart.rx(0x41);              // feed a received byte in ('A')
cpu.uart.rxBytes([0x01, 0x02]); // queue several - delivered one at a time
                                  // as firmware clears RI, matching a real
                                  // UART's single-byte receive buffer
cpu.uart.txLog  // every transmitted byte, in order, for inspection
```

**The one hardware quirk worth knowing:** SBUF is really two physical
registers sharing one address on real 8051s (write = TX buffer, read =
RX buffer) — this file replicates that, so reading `cpu.S0BUF.get()`
after a write does NOT just echo back what you sent; it returns the
last *received* byte, exactly like the real chip.

To actually test a service-port protocol against real tooling (your C++
driver, a terminal), bridge it to a real/virtual COM port with Web
Serial in the browser demo:

```js
const port = await navigator.serial.requestPort();
await port.open({ baudRate: 9600 }); // baud is between the OS and the
                                      // port, doesn't need to match
                                      // anything the firmware set
const bridge = connectWebSerialUart(cpu.uart, port);
// ... run the CPU ...
await bridge.stop(); // when done
```

Verified end-to-end: TX through real `MOV SBUF,#...` opcodes with TI
polling, RX injection with RI/SBUF split-register behavior, 3-byte RX
queueing delivered on successive RI-clears, and TB8/RB8 9-bit pass-through.

## Not implemented

Timer2/capture-compare, PWM, and watchdog/T3 (aside from the standalone
TC1232 model above, which doesn't hook into T3) all have their SFRs
