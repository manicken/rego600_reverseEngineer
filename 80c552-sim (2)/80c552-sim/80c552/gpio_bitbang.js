 ...
 * @param {_51cpu} cpu
 * @param {DS1302} dev
 * @param {{port: reg, ceBit:number, sclkBit:number, ioBit:number}} pins
 */
function install_ds1302(cpu, dev, pins) {
    const { port, ceBit, sclkBit, ioBit } = pins
    watchPin(port, ceBit, { onRise: () => dev.ceRise(), onFall: () => dev.ceFall() })
    watchPin(port, sclkBit, {
        onRise: () => dev.sclkRise(readPinLatch(port, ioBit)),
        onFall: () => dev.sclkFall(),
    })
    drivePinInput(port, ioBit, () => dev._phase === "read" ? dev._outBit : readPinLatch(port, ioBit))
}
...
function install_4094(cpu, dev, pins) {
    const { port, dataBit, clkBit, strBit } = pins
    watchPin(port, clkBit, {
        onRise: () => { dev.shiftReg = ((dev.shiftReg << 1) | readPinLatch(port, dataBit)) & 0xFF },
    })
    watchPin(port, strBit, {
...
function install_4021(cpu, dev, pins) {
    const { port, plBit, clkBit, qBit } = pins
    watchPin(port, plBit, {
    ...
    drivePinInput(port, qBit, () => (dev.shiftReg >> 7) & 1)
}
