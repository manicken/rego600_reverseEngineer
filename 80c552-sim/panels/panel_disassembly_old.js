...
function editCode() {
    ...
    if (disasmLineContext && disasmLineContext.data) {
        AppWindows.Singletons.HexEditor.editor.jumpTo(disasmLineContext.data.addr);
    }
}
...
function gotoLabel() {
    AppWindows.Singletons.GotoLabelForm.open();
}

function showAddressReferences() {
    AppWindows.Singletons.LabelReferencesForm.open(insn_map, disasmLineContext.data.addr);
}
